...
class SupabaseMCPTester {
  ...
  async testProjectsAPI() {
    // Treat Projects API as optional: log warnings but do not fail the whole suite
    if (!this.accessToken) {
      this.log('SUPABASE_ACCESS_TOKEN is not set; skipping Projects API test (optional).', 'warning');
      return {
        message: 'Projects API test skipped: SUPABASE_ACCESS_TOKEN not configured',
        skipped: true
      };
    }
    ...
    try {
      ...
      if (!response.ok) {
        this.log(`Projects API responded with non-OK status: ${response.status} ${response.statusText} (treated as warning).`, 'warning');
        return {
          message: 'Projects API reachable but returned non-OK status',
      ...
      this.log(`Projects API network error (treated as warning): ${error.message}`, 'warning');
      return {
        message: 'Projects API network error (ignored for MCP readiness)',
        error: error.message
      };
    }
  }

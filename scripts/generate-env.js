...
try {
  ...
  console.log('🔎 Loaded .env from project root for env.js generation');
} catch (e) {
  // eslint-disable-next-line no-console
  console.log('ℹ️ dotenv not loaded or .env missing (this is fine on Netlify)');
}
...
function generateEnvFile() {
    console.log('🔄 Generating environment variables for browser...');
    ...
    ENV_VARS.forEach(key => {
        ...
        if (value) {
            env[key] = value;
            hasAnyKey = true;
            console.log(`✅ Found ${key}: ${key.includes('KEY') ? '[REDACTED]' : ('' + value).slice(0, 30) + (("" + value).length > 30 ? '...' : '')}`);
        } else if (Object.prototype.hasOwnProperty.call(FALLBACKS, key)) {
            env[key] = FALLBACKS[key];
            console.log(`ℹ️  Using default for ${key}: ${FALLBACKS[key]}`);
        } else {
    ...
    if (!hasAnyKey) {
        console.warn('⚠️  No environment variables found. Make sure to set them in Netlify dashboard.');
    }
...
`;
    ...
    try {
        ...
        if (fs.existsSync(distDir)) {
            ...
            console.log(`✅ Also copied to dist: ${distPath}`);
        }

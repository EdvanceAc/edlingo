...
async function executeUpsertMigration() {
  try {
    console.log('Executing upsert_lessons_with_materials migration...');
    
    // Read the migration file
    const migrationPath = path.join(__dirname, 'database', 'migrations', '026_upsert_lessons_with_materials_function.sql');
    ...
    if (error) {
      ...
      // Try alternative approach - execute function creation directly
...
GRANT EXECUTE ON FUNCTION upsert_lessons_with_materials(UUID, JSONB) TO authenticated;
    ...
    const { data: testData, error: testError } = await supabase
      .rpc('upsert_lessons_with_materials', {
        p_course_id: '00000000-0000-0000-0000-000000000000', // dummy UUID for test
        p_lessons: []
      });
    
    if (testError && testError.code !== 'PGRST202') {
      console.log('Function is available (got expected error for dummy data)');
    } else if (testError && testError.code === 'PGRST202') {
      console.error('Function still not found after creation attempt');
    } else {

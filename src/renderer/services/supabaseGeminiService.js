                      ...
                      for (const line of lines) {
                        if (line.startsWith('data: ')) {
                          try {
                            ...
                            // Gracefully handle error events from the stream (e.g. PROHIBITED_CONTENT)
                            if (data.error) {
                              console.warn('[SupabaseGeminiService] Live conversation SSE error event:', data.error);
                              const safeMessage = String(data.error).includes('PROHIBITED_CONTENT')
                                ? "I can't respond to that because it may violate safety guidelines. Try rephrasing or asking about another topic."
                                : "I couldn't generate a response just now. Please try asking again or rephrasing your question.";
                              fullResponse = safeMessage;
                              yield {
                                chunk: safeMessage,
                                fullResponse,
                                done: true
                            ...
                            const hasContent = data.content && !data.done;
                            const hasAudio = data.audioData;
                            if (hasContent) {
                              fullResponse += data.content;
                  ...
                  while ((newlineIndex = buffer.indexOf('\n\n')) !== -1) {
                    ...
                    if (line.startsWith('data: ')) {
                      try {
                        ...
                        // Gracefully handle error events mid-stream (e.g. PROHIBITED_CONTENT)
                        if (data.error) {
                          console.warn('[SupabaseGeminiService] Live conversation SSE error event:', data.error);
                          const safeMessage = String(data.error).includes('PROHIBITED_CONTENT')
                            ? "I can't respond to that because it may violate safety guidelines. Try rephrasing or asking about another topic."
